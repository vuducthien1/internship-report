...
import { printReportPdf } from '../../utils/reportPrint';
...
function Reports() {
    ...
    const exportPdf = (report) => {
        ...
        const opened = printReportPdf(report, {
            locale,
            labels: {
                documentTitle: vi ? 'BÁO CÁO TIẾN ĐỘ CÔNG TRƯỜNG' : 'CONSTRUCTION PROGRESS REPORT',
                engineer: t('engineer'),
                ...
                noReviewNote: t('noReviewNote'),
                notReviewed: vi ? 'Chưa duyệt' : 'Not reviewed',
                notAvailable: '—',
                engineerSignature: vi ? 'Kỹ sư lập báo cáo' : 'Reporting engineer',
                reviewerSignature: vi ? 'Người duyệt báo cáo' : 'Report reviewer',
                electronicSignature: t('electronicSignature'),
                verificationText: vi
                    ? 'Thông tin duyệt được xác nhận từ dữ liệu lưu trong hệ thống VDCMS, mã báo cáo'
                    : 'Approval information is confirmed from the VDCMS record, report ID',
                generatedAt: vi ? 'Tạo từ VDCMS lúc' : 'Generated by VDCMS at',
                approval: {
                    pending: t('approvalPending'),
                    approved: t('approvalApproved'),
                    rejected: t('approvalRejected'),
        ...
        if (!opened) setError(vi ? 'Trình duyệt đã chặn cửa sổ xuất PDF.' : 'The browser blocked the PDF window.');
    };
                        ...
                        {selected.approval_status === 'pending' && user?.role !== 'engineer' ? (
                            <section className="report-review-panel">
                                ...
                                <div className="modal-actions report-dialog-actions">
                                    <button type="button" className="btn btn-outline" onClick={() => exportPdf(selected)}>{t('exportPdf')}</button>
                            ...
                            <>
                                ...
                                <div className="modal-actions report-dialog-actions">
                                    <button type="button" className="btn btn-primary" onClick={() => exportPdf(selected)}>{t('exportPdf')}</button>
                                    <button type="button" className="btn btn-outline" onClick={() => setSelected(null)}>{t('cancel')}</button>
                                </div>
                            </>
                        )}
